...
export const App: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-luxury-black text-gray-200 font-sans selection:bg-gold-500 selection:text-black w-full overflow-x-hidden">
      ...
      {showMemberPopup && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/80 backdrop-blur-sm animate-fade-in p-4">
          <div className="bg-luxury-dark border border-gold-500 p-8 rounded-2xl text-center w-full max-w-sm shadow-2xl relative overflow-hidden">
      ...
      {/* Navbar */}
      <nav className={`fixed w-full z-50 transition-all duration-300 ${isScrolled || mobileMenuOpen ? 'bg-black/90 backdrop-blur-lg border-b border-white/10 py-3 md:py-4' : 'bg-transparent py-4 md:py-6'}`}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 flex justify-between items-center">
          <div className="text-2xl font-serif font-bold text-white tracking-tighter cursor-pointer z-50" onClick={() => scrollToSection('root')}>
          ...
          {/* Mobile Menu Toggle */}
          <button className="md:hidden text-white p-2 z-50" onClick={() => setMobileMenuOpen(!mobileMenuOpen)}>
            {mobileMenuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </div>
      </nav>
      
      {/* Mobile Menu Overlay - Strictly Full Screen */}
      {mobileMenuOpen && (
          <div className="fixed inset-0 bg-luxury-black/98 backdrop-blur-xl z-40 flex flex-col items-center justify-center space-y-8 animate-fade-in w-full h-full">
