      ...
      {services.map((service) => (
        <div key={service.id} className="group relative h-96 rounded-2xl overflow-hidden cursor-pointer" onClick={() => onScrollTo('booking')}>
          ...
          <div className="absolute bottom-0 left-0 p-6 w-full transform translate-y-2 group-hover:translate-y-0 transition-transform">
            ...
            <h3 className="text-2xl font-serif text-white mb-1">{service.name}</h3>
            <p className="text-gray-300 text-sm mb-4 opacity-0 group-hover:opacity-100 transition-opacity duration-300 delay-100">{service.category}</p>
            
            <div className="flex justify-between items-center border-t border-white/20 pt-4">
               <span className="text-gold-500 font-bold">₹{service.price}</span>
               <span className="text-white text-sm flex items-center"><Clock size={14} className="mr-1"/> {service.duration}</span>
            </div>
          </div>
        </div>
      ))}
