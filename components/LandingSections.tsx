...
export const HeroSection: React.FC<SectionProps> = ({ onScrollTo }) => (
  <section className="relative w-full min-h-[90vh] md:min-h-screen flex items-center justify-center overflow-hidden pt-20 pb-10">
    {/* Background - Absolute & Zero Z-Index */}
    ...
    {/* Content Container - 100% width on mobile with padding */}
    <div className="relative z-10 w-full px-4 sm:px-6 md:max-w-5xl mx-auto flex flex-col items-center justify-center text-center">
      <div className="inline-block mb-6 px-4 py-2 rounded-full border border-gold-500/30 bg-gold-500/10 backdrop-blur-md">
        <span className="text-gold-500 text-[10px] sm:text-xs font-bold tracking-widest uppercase">Award Winning Luxury</span>
      </div>
      
      {/* Responsive Typography: 4xl mobile -> 7xl desktop */}
      <h1 className="text-4xl sm:text-6xl md:text-7xl lg:text-8xl font-serif text-white mb-6 tracking-tight leading-tight">
        Not Just a Salon. <br />
        <span className="italic text-gold-400">A Masterpiece.</span>
      </h1>
      
      <p className="text-gray-300 text-sm sm:text-base md:text-xl mb-8 md:mb-12 max-w-xl md:max-w-2xl mx-auto font-light leading-relaxed px-2">
      ...
      <div className="flex flex-col w-full sm:w-auto sm:flex-row gap-4 justify-center items-center px-2">
      ...
      {/* Floating Review Badge - Hidden on mobile, visible lg+ */}
      <div className="absolute top-1/2 right-10 transform -translate-y-1/2 hidden lg:block animate-slide-up" style={{ animationDelay: '0.5s' }}>
...
export const LiveWaitTicker = () => (
  <div className="bg-gold-500 text-black overflow-hidden py-3 relative z-20">
...
export const ServicesGrid: React.FC<ServicesProps> = ({ services, onScrollTo }) => (
  <section id="services" className="py-16 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto scroll-mt-24">
    <div className="flex flex-col md:flex-row justify-between items-end mb-8 md:mb-16 gap-4">
        ...
        <button onClick={() => onScrollTo('booking')} className="hidden md:block text-gold-500 border-b border-gold-500 pb-1 hover:text-white hover:border-white transition-colors text-sm md:text-base shrink-0">View Full Menu</button>
    </div>
    
    {/* Grid: 1 col mobile, 2 cols sm, 4 cols lg */}
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
      {services.map((service) => (
        <div key={service.id} className="group relative h-72 sm:h-80 md:h-96 rounded-2xl overflow-hidden cursor-pointer shadow-lg" onClick={() => onScrollTo('booking')}>
          <img 
            src={service.image} 
            alt={service.name} 
          ...
          <div className="absolute bottom-0 left-0 p-5 w-full transform translate-y-2 group-hover:translate-y-0 transition-transform">
            ...
            <p className="text-gray-300 text-xs md:text-sm mb-4 md:opacity-0 group-hover:opacity-100 transition-opacity duration-300">{service.category}</p>
    ...
    <div className="mt-8 text-center md:hidden">
       <button onClick={() => onScrollTo('booking')} className="text-gold-500 border-b border-gold-500 pb-1 text-sm font-medium">View Full Menu</button>
    </div>
  </section>
);

export const StylistSection: React.FC<StylistProps> = ({ stylists, onScrollTo }) => (
  <section id="stylists" className="py-16 md:py-24 bg-neutral-900/50 scroll-mt-24">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        {stylists.map(stylist => (
          <div key={stylist.id} className="bg-luxury-black border border-white/5 rounded-2xl p-6 hover:border-gold-500/30 transition-colors group">
            ...
            <button className="w-full py-3 border border-white/10 rounded-lg text-white hover:bg-gold-500 hover:text-black hover:border-gold-500 transition-all font-medium" onClick={() => onScrollTo('booking')}>Book with {stylist.name.split(' ')[0]}</button>
          </div>
        ))}
      </div>
    </div>
...
export const MembershipCard: React.FC<MembershipProps> = ({ onJoinClick }) => (
  <section id="membership" className="py-16 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto scroll-mt-24">
    <div className="glass-panel rounded-3xl p-8 md:p-16 relative overflow-hidden text-center">
      <div className="absolute top-0 right-0 w-full h-full bg-gradient-to-r from-transparent via-gold-500/10 to-transparent transform rotate-45 scale-150 animate-pulse pointer-events-none"></div>
      ...
      <p className="text-gray-300 max-w-2xl mx-auto mb-10 text-base md:text-lg px-2">
      ...
      <div className="flex flex-col sm:flex-row justify-center gap-4 sm:gap-6 w-full sm:w-auto">
        <button className="bg-gold-500 text-black px-10 py-4 rounded-full font-bold text-lg hover:bg-white transition-colors w-full sm:w-auto" onClick={onJoinClick}>
          Become a Member
        </button>
        <button className="text-white underline hover:text-gold-500 transition-colors mt-2 sm:mt-0 py-2" onClick={() => alert("Demo: Showing benefits modal...")}>
          View Benefits
        </button>
      </div>
    </div>
...
export const ContactForm = () => {
    ...
    return (
        <section id="contact" className="py-16 md:py-24 px-4 sm:px-6 max-w-7xl mx-auto border-t border-white/5">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-start">
                <div>
                    <h2 className="text-3xl md:text-4xl font-serif text-white mb-4">Get in Touch</h2>
                    <p className="text-gray-400 mb-8 text-sm md:text-base">Have a question or special request? Send us a message and our concierge will get back to you shortly.</p>
                    <div className="space-y-4 text-gray-400">
                         <div className="flex items-center gap-4 p-4 bg-white/5 rounded-lg">
                            <Phone className="text-gold-500 shrink-0" />
                            <div className="overflow-hidden">
                                <p className="text-xs text-gray-500 uppercase">Phone</p>
                                <p className="text-white truncate">+1 (555) 123-4567</p>
                            </div>
                         </div>
                         <div className="flex items-center gap-4 p-4 bg-white/5 rounded-lg">
                            ...
                            <div className="overflow-hidden">
                                ...
                                <p className="text-white truncate">concierge@lumieresalon.com</p>
                            </div>
                         </div>
                    </div>
                </div>

                <form onSubmit={handleSubmit} className="glass-panel p-6 md:p-8 rounded-2xl space-y-4 w-full">
...
export const Footer: React.FC<SectionProps> = ({ onScrollTo }) => (
    <footer className="bg-black pt-12 pb-12 border-t border-white/10 text-center md:text-left">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8 md:gap-12 mb-16">
            ...
            <div>
                ...
                <ul className="space-y-4 text-gray-500 text-sm flex flex-col items-center md:items-start">
                    ...
                    <li className="flex items-center text-green-500"><div className="w-2 h-2 rounded-full bg-green-500 mr-2"></div> Open Now</li>
                </ul>
            </div>
            <div>
                <h3 className="text-white font-bold mb-6">Quick Links</h3>
                <ul className="space-y-3 text-gray-500 text-sm">
                    <li><button onClick={() => onScrollTo('services')} className="hover:text-gold-500 transition-colors py-1">Services Menu</button></li>
                    <li><button onClick={() => onScrollTo('booking')} className="hover:text-gold-500 transition-colors py-1">Book Appointment</button></li>
                    <li><button onClick={() => onScrollTo('stylists')} className="hover:text-gold-500 transition-colors py-1">Stylist Portfolio</button></li>
                    <li><button onClick={() => onScrollTo('membership')} className="hover:text-gold-500 transition-colors py-1">Membership</button></li>
                </ul>
            </div>
            <div>
                 ...
                 <p className="text-gray-600 text-xs">© 2024 Lumière Salon.</p>
            </div>
        </div>
    </footer>
);